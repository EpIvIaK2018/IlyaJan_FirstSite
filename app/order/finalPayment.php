...
session_start();
function sendMail(string $mail, string $message): void{ //ivb-1989@yandex.ru
    ...
    if(mail($mail, "Заказ подтверждён", "$message", $headers)){
        echo " Почта отправлена!";
        header('Refresh: 0.1; URL=../account/myOrders.php');
    }
}

if(!empty($_SESSION['user_id'])){
    $typeOfPayment = $_POST['dostavka'];
    ...
    if($typeOfPayment === 'delivery'){
        ...
        if($ost >= 0){
            ...
            $message = $data['publicName'] . " Заказ оформлен, доставка будет в течение 3 дней" . "Товары:" . $str;
            sendMail($mail, $message);
        }
    }else if($typeOfPayment === 'sam'){
        $stmt = Connect::getLink()->prepare("SELECT `product`, `count` FROM cart_items WHERE id_Customer = {$_SESSION['user_id']}");
        $stmt->execute();
        $str= json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
        $message = $data['publicName'] . " Заказ оформлен, Заказ будет ожидать вас в течение 5 рабочих дней.\n" . "Товары:" . $str;
        try{
            $currentId = "00000" . Connect::getLink()->prepare("SELECT id FROM orders where customer_id = {$_SESSION['user_id']}")->execute();
            $currentId = intval($currentId) + 1;
            $currentTime = date('Y-m-d H:i:s');
